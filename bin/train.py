...
parser.add_argument('--pre_train', default="/content/pcam_pg/config/pre_train.pth", type=str, help="If get"
...
def run(args):
    ...
    model = Classifier(cfg) # model is done
    if args.verbose is True:
        ...
        summary(model.to(device), (3, h, w)) # showing he full model summary
    ...
    for epoch in range(epoch_start, cfg.epoch):
        ...
        time_now = time.time()
        summary_dev, predlist, true_list = test_epoch(
            summary_dev, cfg, args, model, dataloader_dev) # since it is validating after every epoch train what it is necissity of cfg.test_every ?
        time_spent = time.time() - time_now
        ...
        torch.save({'epoch': summary_train['epoch'],
                    ...
                    'loss_dev_best': best_dict['loss_dev_best'],
                    'state_dict': model.module.state_dict()},
                   os.path.join(args.save_path, 'train.ckpt')) # saves the model after every epoch by same name , this can be used for resume training
